...
#pragma omp parallel num_threads(processor_count)
    {
        ...
        auto result = kmp_search(reinterpret_cast<const char *>(base_addr + _offset), _size, pattern, strlen(pattern));
...
auto search_with_openmp_simd(const uint8_t* p, size_t total_length, const char *pattern) -> std::vector<size_t> {
...
auto do_test_in_memory(const int size, const char *pattern, const int count) {
    ...
    auto result1 = search_with_single_thread(p, size, pattern);
    check_print_result(p, size, pattern, result1);

    auto result2 = search_with_single_thread_simd(p, size, pattern);
    check_print_result(p, size, pattern, result2);

    auto result3 = search_with_openmp(p, size, pattern);
    check_print_result(p, size, pattern, result3);

    auto result4 = search_with_openmp_simd(p, size, pattern);
...
int main() {
    do_test_in_memory(2048 * 1024 * 1024, "PATTERN", 5);
    return 0;
}
